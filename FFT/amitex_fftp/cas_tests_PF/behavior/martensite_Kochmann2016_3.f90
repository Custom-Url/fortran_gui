!=======================================================================
!!                      FONCTION MARTENSITE_KOCHMANN2016_3
!!
!! ADAPTATION LG DE MARTENSITE_KOCHMANN2016 A L'INTERFACE "3" ('NLOC')
!!       ASSOCIE A user_Kochman3/non_local_user_mod.f90
!!----------------------------------------------------------------------
...
!!                              "alpha" in user_Kochman3/non_local_user_mod.f90
!!                              Psy_G0 in [Kochmann2016] p.95 expres. 20 or p.96 expres. 28, 
!!                              Value in [Kochmann2016] p.100 table 1
!!        PROPS(32)             kinetic coefficient
!!                             "Kinetic" in user_Kochman3/non_local_user_mod.f90
!!                              m_0 in [Kochmann2016] p.95 expres. 20
!!                              Value in [Kochmann2016] p.100 table 1
!!
!! \param[out] STRESS  (tableau reel(6)) tenseur des contraintes
!!
!! \Intvar[in][out]   STATEV    (tableau reel(4)) variables internes, 
!!                              1-3 -> envoyees dans resolutionPF_mod.f90
!!                              4 -> mise a jour depuis user_nloc_xxx
!!                              (variables mises à jour depuis dans user_nloc_xxx positionnees en fin de vecteur)
!!
!!
!!        STATEV(1)   (out) force motrice df/dEta_i pour champ de phase (sans la contribution gradient)
!!                          -> Nloc(1)%Var(:,1) dans user_Kochman3/non_local_user_mod.f90
!!        STATEV(2)   (in)  valeur champ de phase "i" mise a jour depuis user_Kochman3/non_local_user_mod.f90
!!                          -> GNloc(1)%Var(:,1) dans user_Kochman3/non_local_user_mod.f90
!!
!! Notation CAST3M (Voigt, ordre 11 22 33 12 13 23)
!=======================================================================

SUBROUTINE martensite_Kochmann2016_3( STRESS, STATEV, DDSDDE, SSE, SPD, SCD,&
...
!-----------------------------------------------------------------------
  ...
  CE = PROPS(1:9) + STATEV(2)*STATEV(2)*(PROPS(16:24) - PROPS(1:9))

  ! self deformation intrinsic to phase tranformation 
  ! associated with phase field "i" STATEV(i)
  !ER = STATEV(2)*PROPS(31:36)
  ! compute bain / residual strain in case of martensite transformations
  ! here from [Kochmann2016] p.99 expres.(43) 
  ER(1) = STATEV(2)*STATEV(2)*PROPS(25)
  ER(2) = ER(1) 
  ER(3) = 0._8
  ER(4) = STATEV(2)*PROPS(28)*2._8
  ER(5) = 0._8
  ER(6) = 0._8
  ...
  dER_dEta(1) = 2._8*STATEV(2)*PROPS(25)
  ...
  dCE_dEta = 2._8*STATEV(2)*(PROPS(16:24) - PROPS(1:9))
  ...
  STATEV(1) =  STATEV(1) - f_homo * 12._8*STATEV(2)*(1._8 - STATEV(2)*STATEV(2)) &
                                  *(a*a - STATEV(2)*STATEV(2)) &
                                  /(3*a*a - 1._8)


end subroutine martensite_Kochmann2016_3

!===============================FUNCTIONS BELOW AVAILABLE IN martensite_Kochmann2016.f90===============
!                    uncomment if martensite_Kochmann2016.f90 is not present
