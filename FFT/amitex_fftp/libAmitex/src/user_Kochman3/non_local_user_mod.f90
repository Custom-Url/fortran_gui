...
!==================================================================================
!      KOCHMANN : INIT_USER_NLOC_VARIABLES1 & USER_NLOC_AFTER1
!
!> Implementation of the Kochmann 2016 model
!! 
!!    with the behavior martensite_Kochmann2016_3
!!
...
subroutine user_nloc_after1(Nnloc,NGnloc,list_mat,list_varInt_Nloc,list_varInt_GNloc,modelInd,&
 ...
 ! get df/dEta from MattotP%Varint
 !----------------------------------------
 
 call Assemble_field_from_VarInt(list_mat,list_varInt_Nloc,Nnloc,Nloc(modelInd)%Var)    !-> df_deta

 ! evaluate (Eta - L.dt.df_dEta) -> Eta
 !----------------------------------------
 ...
 ! schema semi implicte
 ! EtaF = EtaF  / (1+Freq^2 alpha.dt) 
 !----------------------------------------
 
 EtaF = EtaF/(1._mytype + FREQ2 * Kinetic * alpha * dt)
 ...
 ! store Eta in MattotP%Varint
 !----------------------------------------
 
 call Assign_VarInt_from_Field(list_mat,list_varInt_GNloc,NGnloc,GNloc(modelInd)%Var) !<-Eta 
