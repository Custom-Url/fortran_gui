...
!----------------
  ...
  !> USER variables  
  real(mytype)                              :: alpha, kinetic   ! coefficients (homogenes) du champ de phase
  ...
  complex(mytype),pointer,dimension(:,:,:)  :: EtaF           !
...
!==================================================================================
!       CAHNHILLIARD3 : INIT_USER_NLOC_VARIABLES, USER_NLOC_AFTER1
!
...
!!
!!     COEFF : 
!!          alpha   : 1st non-local coeff
!!          kinetic : 2nd non-local coeff
!!
!!     NLOC : 
!!          df_dEta : Nloc(modelInd)%Var(:,1)
!!
!!     GNLOC : 
!!          Eta     : GNloc(modelInd)%Var(:,1)
!!          EtaF    : GNloc(modelInd)%VarF(:,:,:,1)
!!
!!
!! LIMITATIONS OF THIS IMPLEMENTATION : 
!!        HOMOGENEOUS PHASE-FIELD COEFF (alpha and kinetic)
!!
!==================================================================================
...
subroutine init_user_nloc_variables1(Nnloc,NGnloc,list_mat,list_varInt_Nloc,list_varInt_GNloc,modelInd)
 ...
 !  alpha   : 1st non-local coeff
 !  kinetic : 2nd non-local coeff
 !  get_homogeneous_coeff check also if input coefficients are homogeneous
 
 call get_homogeneous_coeff(alpha,  modelInd,  1, list_mat)
 call get_homogeneous_coeff(kinetic, modelInd, 2, list_mat)
...
subroutine user_nloc_after1(Nnloc,NGnloc,list_mat,list_varInt_Nloc,list_varInt_GNloc,modelInd,&
 ...
 if(load_incr/=0) then ! Pas de sortie vtk pour les pas de temps fictifs
     ...
     tmp_char="_eta_"//trim(adjustl(tmp_char1))   
     call print_field_vtk(Eta,trim(fic_vtk)//trim(tmp_char)//".vtk","eta", ind_tps) 
                
 end if
 
 !================================================================================
 !                                                     EVOLUTION DU CHAMP DE PHASE

 ! GET df/dEta from MattotP%Varint (via Nloc%Var)
 ! GET Eta from MattotP%Varint (via GNloc%Var)
 !--------------------------------------------------
 call Assemble_field_from_VarInt(list_mat,list_varInt_Nloc,Nnloc,Nloc(modelInd)%Var)     !-> df_deta (pointer on Nloc(modelInd)%Var(:,1))
 call Assemble_field_from_VarInt(list_mat,list_varInt_GNloc,NGnloc,GNloc(modelInd)%Var)  !-> Eta  (pointer on GNloc(modelInd)%Var(:,1))

 ! FOURIER TRANSFORM (df_dEta)     -> stored in EtaF
 !--------------------------------------------------
 call field_fft(df_dEta,EtaF,1,1) 
 EtaF=EtaF/ real(grid%ntot,mytype)  

 ! LAPLACIAN  (x-1)
 ! EtaF = - (Freq^2) * EtaF        -> stored in EtaF
 !--------------------------------------------------
 EtaF = FREQ2(:,:,:) * EtaF

 ! INVERSE FOURIER TRANSFORM    -> stored in df_dEta
 !--------------------------------------------------
 call field_ifft(df_dEta,EtaF,1,1)

 ! Eta0 + L.dt.Laplacien[df_dEta]          -> in Eta
 !--------------------------------------------------
 Eta = Eta - Kinetic*dt*df_dEta

 ! FOURIER TRANSFORM                      -> in EtaF
 !--------------------------------------------------
 call field_fft(Eta,EtaF,1,1) 
 EtaF=EtaF/ real(grid%ntot,mytype)

 ! EtaF = EtaF  / (1 + (Freq^2)^2 alpha.dt) -> in EtaF
 !--------------------------------------------------
 EtaF = EtaF/(1._mytype + FREQ2 * FREQ2 * Kinetic * alpha * dt)

 ! INVERSE FOURIER TRANSFORM                  -> Eta
 !--------------------------------------------------
 call field_ifft(Eta,EtaF,1,1)
