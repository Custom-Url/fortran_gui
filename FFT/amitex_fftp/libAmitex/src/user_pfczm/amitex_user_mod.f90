...
!       -   BB : viscous term, BB = 2*HH/lc/gc + visc/dt/lc/gc*damageVar
  ...
  integer                   :: id_visc, id_lc, id_gc !need to be consistent with "mat_*.xml"
  integer,parameter         :: id_psiP=2             !need to be consistent with "mat_*.xml"
  !                                                                   Loop of every voxel
  ! -------------------------------------------------------------------------------------
  do i=1,size(mattotP)
     ...
     if (trim(mattotP(i)%LawName_comp(1))=='elasisoPF') then
        id_visc=3   !need to be consistent with "mat_*.xml"
        id_lc=4
        id_gc=5
     elseif (trim(mattotP(i)%LawName_comp(1))=='elasanisoPF') then
        id_visc=10   !need to be consistent with "mat_*.xml"
        id_lc=11
        id_gc=12
     end if
     !indice minimum de zone
