...
volume_fractions = [0.40]
n_geometries = 5 
porosity_targets = [0.0]
...
def convert_alpha_3d(alpha2d):
    nz, _ = alpha2d.shape
    alpha3d = np.repeat(alpha2d[np.newaxis, :, :], nz, axis = 0)
    return alpha3d
    


def write_vtk_structured_points(filename, alpha3d, spacing, pore_mask=None):
    ny, nx, nz = alpha3d.shape
    nx_points, ny_points, nz_points = nx + 1, ny + 1, nz + 1
    n_cells = nx * ny * nz

    if pore_mask is None:
        pore_mask = np.zeros((n + 1, n + 1), dtype=np.uint8)

    volume3d = alpha3d.astype(np.uint8)
    volume3d += pore_mask
    volume3d[volume3d == 2] = 0 #Map Fibre/Pore Overalp to Fibre
    volume3d[volume3d == 1] = 2 # Map Matrix to 2 as expected by AMITEX
    volume3d[volume3d == 0] = 1 # Map Fibre to 1 as expected by AMITEX

   # volume = np.reshape(volume2d, (1, ny, nx)).astype(np.uint8)

    with open(filename, 'wb') as f:
        f.write(b"# vtk DataFile Version 4.5\n")
        f.write(b"3D Fibre Alpha for AMITEX\n")
        f.write(b"BINARY\n")
        ...
        f.write(volume3d.tobytes(order='C'))
...
for vf in volume_fractions:
    for i in range(n_geometries):
        ...
        for target_porosity in porosity_targets:
            ...
            subdir = f"mesh_3D/L{domain_str}_vf{vf:.2f}/h{spacing_str}"
            os.makedirs(subdir, exist_ok=True)
            
            
            filename = os.path.join(subdir, f"iUC{i + 1}_vpMIN{target_porosity}.vtk")
            alpha_bin_3d = convert_alpha_3d(alpha_bin)
            write_vtk_structured_points(filename, alpha_bin_3d, spacing, pore_mask)
